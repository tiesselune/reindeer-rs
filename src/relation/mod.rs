...
impl Relation {
    ...
    pub fn relations_with_key<E1: Entity>(key : &[u8], db: &Db) -> std::io::Result<RelationDescriptor> {
    ...
    fn get_descriptor_with_key_and_tree_name(tree_name: &str, e : &[u8], db : &Db) -> std::io::Result<RelationDescriptor> {
    ...
    fn save_descriptor_with_key_and_tree_name(tree_name : &str, e : &[u8], r_d : &RelationDescriptor, db : &Db) -> std::io::Result<()> {
        let tree = db.open_tree(Relation::tree_name(tree_name))?;
        tree.insert(e, bincode::serialize(r_d).unwrap())?;
        Ok(())
    }
